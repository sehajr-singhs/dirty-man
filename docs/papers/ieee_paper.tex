...
\url{https://github.com/sehajr-singhs/switch-operator} --- every number is read from committed JSON result files by \texttt{make\_figs.py}; the benchmark is procedural (no downloads); \texttt{--device cuda} forces GPU execution and \texttt{run\_experiments.py --smoke} verifies the pipeline in ~1 minute.
